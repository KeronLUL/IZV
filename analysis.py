...
def plot_roadtype(df: pd.DataFrame, fig_location: str = None,
    ...
    df_type = df[df['region'].isin(['JHC', 'JHM', 'HKK', 'KVK'])][['region', 'p21']]
    df_type['tmp'] = 1
...
def plot_animals(df: pd.DataFrame, fig_location: str = None,
    ...
    df_animals = df_animals[df_animals['region'].isin(['JHC', 'JHM', 'HKK', 'KVK'])][['region', 'p10', 'date']]
...
def plot_conditions(df: pd.DataFrame, fig_location: str = None,
    ...
    df_w = df[df['region'].isin(['JHC', 'JHM', 'HKK', 'KVK'])][['region', 'p18', 'date']]
    df_w = df_w[(df_w['date'] >= pd.to_datetime('2016-1-1')) & (df_w['date'] <= pd.to_datetime('2020-12-31'))]
    df_w = df_w[df_w['p18'] != 0]
    df_w['p18'] = df_w['p18'].map(weather)
    df_w['tmp'] = 1
    df_w = df_w.pivot_table(columns='p18', values='tmp', index=['date', 'region'], aggfunc='sum').reset_index()
    df_w = df_w.fillna(0)
    df_w = df_w.groupby([pd.Grouper(key='date', freq='M'), 'region']).agg('sum').reset_index()
    df_w = df_w.melt(id_vars=["region", 'date'], var_name="p18", value_name="value")
