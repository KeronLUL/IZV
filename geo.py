...
def plot_cluster(gdf: geopandas.GeoDataFrame, fig_location: str = None,
    ...
    gdf_plot = gdf_plot.dissolve(by="cluster", aggfunc={"p1": "count"}).rename(columns=dict(p1="count"))

    plt.figure(figsize=(10, 10))
    plt.suptitle('Nehody v Jihomoravském kraji na silnicích 1. třídy')
    ax = plt.gca()

    gdf_plot.plot(ax=ax, markersize=8, column="count", legend=True, alpha=0.6, legend_kwds={'location': 'bottom', 'label': 'Počet nehod v úseku', 'pad': 0.01})
